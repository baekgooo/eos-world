...
function applyFlowHighlight(){{
  ...
  nodes.forEach(n=>{{
    ...
    n.classList.toggle('previous',id===previousId&&id!==selectedId);
...
function selectSection(id){{const s=byId.get(id);if(!s)return;if(selectedId&&selectedId!==id)previousId=selectedId;selectedId=id;applyFlowHighlight();readerId.textContent=`${{s.id}} · ${{s.kind==='decision'?'분기발생지점':s.kind==='ending'?'엔딩':'스토리'}}`;readerTitle.textContent=s.title;openSmall.href=s.smallUrl;readerBody.style.display='block';document.querySelectorAll('.info-panel').forEach(p=>p.classList.remove('active'));readerBody.innerHTML=s.html+`<section class="illust-card"><strong>삽화 메모</strong><p>${{s.illustration}}</p></section>`;readerBody.scrollTo({{top:0,behavior:'smooth'}});document.querySelector(`[data-section="${{id}}"]`)?.scrollIntoView({{block:'center',inline:'center',behavior:'smooth'}});requestAnimationFrame(renderEdges);}}
