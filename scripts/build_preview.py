...
function renderEdges(){{
  ...
  for(const edge of edges){{
    ...
    if(sameColumn&&longJump){{
      ...
      const sourceSide=a.classList.contains('decision')&&Math.abs(bx-ax)>=10?Math.sign(bx-ax):0;
      const start=sourceSide?point(ar,a,'side',sourceSide):point(ar,a,downward?'bottom':'top');
